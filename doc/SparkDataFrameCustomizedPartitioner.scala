...
class DomainNamePartitioner(numParts: Int) extends Partitioner {
  ...
  override def getPartition(key: Any): Int = {
    ...
    if(key==null){
     ...
     code  =r2.nextInt(6)+3// (key.hashCode % (numPartitions-3)+3)
    }
    code
  }
...
object SparkPartitioner {
  ...
  def main(args: Array[String]) {
    ...
    val pairRDD=mergedDF.rdd.map(row=>(row(6),row))
    val customizedPairRDD=pairRDD.partitionBy(new DomainNamePartitioner(9))
   ...
   customizedDF.show(false)
     ...
     customizedDF.rdd.mapPartitionsWithIndex{case (i,rows) => Iterator((i,rows.size))}.toDF("partition_number","number_of_records").show(40)
